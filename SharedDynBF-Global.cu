...
#define SUB_STR 5
#define MAIN_STR 1181741

static __constant__ char d_stringPattern[BLOCK_SIZE];

__global__ void compare(int idx,const char* x,const char* y,int results[],int shift) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
//			printf("%d %c %c\n",shift,x[id],y[shift+idx]);
	if(x[id]!=y[shift+id]) {
		results[shift]=0;
	}
}
...
//			strcpy(c,sharedData);
	...
	//		printf("%d %d \n",shft,results[shft]);
		...
		if((s2[0] == sharedData[threadIdx.x]) && (s2[SUBSTRLEN-1] == sharedData[threadIdx.x + SUBSTRLEN - 1])) {
					compare<<<1,SUBSTRLEN>>>(threadIdx.x,substr,data,results,shft);
//			printf("%d\n",shft);
		} else {
				results[shft]=0;
		}
...
//    cutilSafeCall( cudaMemcpy(d_stringPattern, subString, sizeof(char)*(strlen(subString)), cudaMemcpyHostToDevice ) );
...
//	    strmatch<char*, m, n><<<numBlocks,threadsPerBlocks>>>(d_substr, d_data, strlen(mainString), strlen(subString), d_finalres);
	    strmatch<char*, SUB_STR, MAIN_STR><<<numBlocks,threadsPerBlocks>>>(d_substr, d_data, strlen(mainString), strlen(subString), d_finalres);
	// stop timer
